...
void setColorPalette (uint8_t color, uint8_t red, uint8_t green, uint8_t blue)
{
	...
	uint8_t index = color >> 2;
	uint32_t redshifted = ((uint32_t) red) << 8;
	uint32_t greenshifted = ((uint32_t) green) << 4;
	uint32_t add = redshifted + greenshifted + blue;
	if (color % 2 == 1) {
		hdmi_ctrl->palette[index] &= 0x0000FFFF;
		add = add << 16;
	} else {
		hdmi_ctrl->palette[index] &= 0xFFFF0000;
	}
	hdmi_ctrl->palette[index] += add;
}
