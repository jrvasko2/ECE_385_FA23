...
{
    SPI_init(); //initialize the microblaze spi interface
...
{
	u8* writebuffer;
	*writebuffer = BURST_FIFO_READ;
	XSpi_Transfer(&SpiInstance, writebuffer, NULL, 1);
...
uint8_t cameraReadByte(ArducamCamera* camera)
{
    u8 readbuffer[3];
    u8 regbuffer[3];
    regbuffer[0] = SINGLE_FIFO_READ;
    regbuffer[1] = 0;
    regbuffer[2] = 0;
    XSpi_SetSlaveSelect(&SpiInstance, SpiInstance.SlaveSelectMask);
    XSpi_Transfer(&SpiInstance, regbuffer, readbuffer, 3);
    XSpi_SetSlaveSelect(&SpiInstance, 0); //set SS low
...

{
    ...
    *writebuffer = 0x00;
    ...
    for (uint32_t count = 0; count < length; count++) {
        XSpi_Transfer(&SpiInstance, writebuffer, buff + count, 1);
    }
#else
...
uint8_t cameraBusWrite(ArducamCamera* camera, int address, int value)
{
	...
	writebuffer[0] |= (1<<7); //set MSB of command byte to 1 for write?
	writebuffer[1] = value;
	int error_code = XSpi_Transfer(&SpiInstance, writebuffer, NULL, 2);
	//write val via SPI
	//read return code from SPI peripheral (see Xilinx examples)
	//int error_code = XSpi_Transfer(&SpiInstance, &val, NULL, 1);
	//if return code != 0 print an error
	if (error_code != 0) {
		xil_printf("reg write failed %d", error_code);
	}
	//deselect camera (may not be necessary if you are using SPI peripheral)
    XSpi_SetSlaveSelect(&SpiInstance, 0); //set SS low
    return 1;
}
...
uint8_t cameraBusRead(ArducamCamera* camera, int address)
{
	...
	regbuffer[1] = 0;
	regbuffer[0] = address;
	int e1 = XSpi_Transfer(&SpiInstance, regbuffer, readbuffer, 3);
	//read return code from SPI peripheral
	//if return code != 0 print an error
	if (e1 != 0) {
		xil_printf("Error: read failed %d", e1);
	}
	//deselect camera (may not be necessary if you are using SPI peripheral)
